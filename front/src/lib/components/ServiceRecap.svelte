...
<div class="bg-white w-full md:w-auto p-6 rounded-xl shadow-lg max-w-4xl mx-auto text-black">
  <div class="flex flex-col md:flex-row justify-between items-center space-y-6 md:space-y-0 md:space-x-8">
    <div class="w-full md:w-1/2">
      <h2 class="text-2xl font-semibold mb-2">{contenu[lang].selectService}</h2>
      <div class="relative">
        <select
        ...
        >
          <option value="" disabled selected>{contenu[lang].chooseService}</option>
    ...
    <div class="w-full md:w-1/2 text-center md:text-right">
      <p class="text-xl mb-2">{contenu[lang].totalPrice}</p>
