<script lang="ts">
  ...
  function nextStep(service: string, price: number) {
    goto("/reserve?"+ new URLSearchParams({
      lang: lang, 
      service: service, 
...
layduhurdevelopment@gmail.com");
  ...
  function handleKeyDown(event: KeyboardEvent) {
    if (departInput.getShowAutocomplete() || (arrivalInput && arrivalInput.getShowAutocomplete())) {
      return;
    }
  ...
  function handleClickOutside(event: MouseEvent) {
    ...
    if (!target.closest('#depannageType') && !target.closest('.depannage-dropdown')) {
    ...
    <div class="section min-h-screen flex flex-col relative transition-colors duration-1000">
      ...
      <div class="w-full flex-grow" data-aos="fade-up" data-aos-duration="1500">
        <section class="max-w-7xl mx-auto  mt-24 px-4 sm:px-8">
          <h2 class="text-5xl font-bold mb-12 text-center md:text-left">{contenu[lang][selectedService].title}</h2>

          <div class="flex justify-center mb-8">
            <div class="grid grid-cols-3   gap-2 bg-black bg-opacity-10 p-2 rounded-lg max-w-full">
              <button 
                class="{selectedService === 'Convoyage' ? 'bg-amstram-purple text-white border-2 border-white' : 'text-gray-300 border border-gray-500'} flex items-center justify-center py-2 px-3 rounded-lg transition-all duration-300"
                on:click={() => selectedService = 'Convoyage'}>
                <Truck class="mr-2" size={20} />
                {contenu[lang]['Convoyage'].name} 
              </button> 
              <button 
                class="{selectedService === 'Dépannage' ? 'bg-amstram-purple text-white border-2 border-white' : 'text-gray-300 border border-gray-500'} flex items-center justify-center py-2 px-3 rounded-lg transition-all duration-300"
                on:click={() => selectedService = 'Dépannage'}>
                
    <div class="relative w-5 h-5 mr-2 flex items-center justify-center">
      ...
      <div class="absolute inset-0 flex items-center justify-center transition-opacity duration-500 ease-in-out opacity-0 transform transition-transform" 
        ...
        <Wrench size={20} />
              ...
              <button 
                class="{selectedService === 'Remorquage' ? 'bg-amstram-purple text-white border-2 border-white' : 'text-gray-300 border border-gray-500'} flex items-center justify-center py-2 px-3 rounded-lg transition-all duration-300"
          ...
          <form class="grid grid-cols-1 md:grid-cols-4 gap-6 ml-10 mr-10">
            ...
            {#if selectedService == "Dépannage"}
            <div class="relative">
              <label for="vehicleType" class="sr-only">{contenu[lang].depannage_type}</label>
              <!-- Bouton qui sert de déclencheur pour le menu déroulant -->
              <button 
                type="button"
                id="depannageType"
                class="w-full pl-4 pr-12 py-3 rounded-lg bg-white text-amstram-black appearance-none text-lg flex items-center justify-between {selectedDepannage ? 'outline outline-2 outline-amstram-purple' : ''}"
                on:click={() => showDepannageDropdown = !showDepannageDropdown}>
                <span >{selectedDepannage ? selectedDepannage : contenu[lang].depannage_type}</span>
                <ChevronDown class="text-gray-400 "/>
              </button>
              
              <!-- Menu déroulant avec icônes -->
              {#if showDepannageDropdown}
                <div class="absolute z-10 mt-1 w-full rounded-lg bg-white shadow-lg py-1 text-amstram-black">
      ...
      <select id="vehicleType" class="w-full pl-4 pr-12 py-3 rounded-lg bg-white text-amstram-black appearance-none text-lg {selectedVehicle ? 'outline outline-2 outline-amstram-purple' : ''}"
        bind:value={selectedVehicle}>
              ...
              <div class="pointer-events-none absolute right-4 top-1/2 transform -translate-y-1/2">
                <ChevronDown class="text-gray-400" /> <!-- CHEVRON POS A DROITE -->
      </div>
    </div>
    <!-- EL BUTTON RESERVER-->
            <button on:click={(event) => { event.preventDefault(); handlePrice(true); }} 
            bind:this={reserveButton}
            disabled={!(selectedVehicle && (arrival_set || (selectedDepannage != "")) && depart_set)} 
            class="{(selectedVehicle && (arrival_set || (selectedDepannage != "")) && depart_set) ? 'bg-amstram-purple scale-105 transition-all duration-1000' : 'bg-black bg-opacity-10'} text-white py-3 px-6 rounded-lg text-lg font-semibold">{contenu[lang].reserve}</button>
          </form>
        </section>

        <section class="max-w-7xl mx-auto mr-20 mt-10 px-4 sm:px-8 flex flex-col md:flex-row items-center">
          <div class="md:w-1/2 mb-12 md:mb-0">
            ...
            <div class="flex mt-8 mb-2 space-x-4">
              <a href="https://www.instagram.com/amstram.eu" target="_blank" rel="noopener noreferrer" 
                 class="p-3  rounded-full hover:bg-opacity-80 transition-all duration-300">
                <InstagramIcon size="24" />
              </a>
              <a href="https://www.facebook.com/profile.php?id=61573754154420" target="_blank" rel="noopener noreferrer"
                 class="p-3  rounded-full hover:bg-opacity-80 transition-all duration-300">
