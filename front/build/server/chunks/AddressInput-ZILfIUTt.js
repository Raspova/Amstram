import { c as create_ssr_component, f as compute_rest_props, g as spread, l as escape_attribute_value, h as escape_object, d as createEventDispatcher, v as validate_component, b as add_attribute, e as escape, o as each } from './ssr-uqARPLdZ.js';
...
const AddressInput = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="relative">${validate_component(Map_pin, "MapPin").$$render(
    $$result,
    {
    ...
    value_set ? "outline outline-2 outline-amstram-purple" : "",
    true
  ) + " " + escape("", true)}"${add_attribute("this", input, 0)}> ${showAutocomplete ? `<ul class="absolute z-10 bg-white border border-gray-300 w-full mt-1 rounded-md shadow-lg">${each(autocompleteResults, (result, index) => {
    return `<button type="button" class="${"cursor-pointer p-2 text-left text-black hover:bg-gray-200 " + escape(selectedIndex === index ? "bg-gray-200" : "", true)}">${escape(result)} </button>`;
  })}</ul>` : ``}</div>`;
});

export { AddressInput as A, Card as C, Icon as I, Card_header as a, Card_title as b, Card_content as c, contenu as d, cn as e };
//# sourceMappingURL=AddressInput-ZILfIUTt.js.map
