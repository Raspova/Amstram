import { c as create_ssr_component, g as compute_rest_props, h as spread, i as escape_attribute_value, j as escape_object, d as createEventDispatcher, v as validate_component, b as add_attribute, e as escape, f as each } from './ssr-CbbsIBsB.js';
...
const AddressInput = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="relative w-full max-w-md mx-auto"><div class="relative">${validate_component(Map_pin, "MapPin").$$render(
    ...
    {},
    {}
  )} <input type="text"${add_attribute("placeholder", contenu[lang].departLocation, 0)} class="${"w-full pl-10 pr-4 py-2 rounded-lg bg-white text-gray-900 text-lg border border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent transition duration-200 ease-in-out " + escape(value_set ? "ring-2 ring-purple-500" : "", true) + " " + escape(precise ? "" : "ring-2 ring-red-500", true) + " " + escape("", true)}"${add_attribute("this", input, 0)}> ${!precise ? `<div class="absolute right-0 top-1/2 transform -translate-y-1/2 mr-2">${validate_component(Circle_alert, "AlertCircle").$$render($$result, { class: "text-red-500 h-5 w-5" }, {}, {})}</div>` : ``}</div> ${showAutocomplete ? `<ul class="absolute z-10 bg-white border border-gray-300 w-full mt-1 rounded-md shadow-lg max-h-60 overflow-y-auto">${each(autocompleteResults, (result, index) => {
  ...
  })}</ul>` : ``} ${!precise ? `<div class="absolute z-20 bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded-md shadow-lg mt-2 right-0">${escape(contenu[lang].needMorePreciseAddress)}</div>` : ``}</div>`;
});

export { AddressInput as A, Card as C, Icon as I, Card_header as a, Card_title as b, Card_content as c, contenu as d, cn as e };
//# sourceMappingURL=AddressInput-Cz3gct4c.js.map
